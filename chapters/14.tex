...
\ea \label{ex:14.5}
\ea    {All students are brilliant}.\\ \jambox{S ${\subseteq}$ B}
\ex   {No students are brilliant}. \\ \jambox{\textsc{S} ${\cap}$ \textsc{B} = ⌀}
\ex   {Some students are brilliant}.\\ \jambox{\textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} ${\geq}$ 2}
\ex  {A/Some student is brilliant}.\\ \jambox{\textsc{S} ${\cap}$ \textsc{B} ≠ ⌀; or:  \textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} ${\geq}$ 1}
\ex   {Four students are brilliant}.\\ \jambox{\textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} = 4\footnotemark{}}
\ex   {Most students are brilliant}.\\ \jambox{\textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} > \textsc{{\textbar}S} – \textsc{B}{\textbar}; or: \textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} > ½\textsc{{\textbar}S}{\textbar}}
\ex    {Few students are brilliant}.\\ \jambox{\textsc{{\textbar}S} ${\cap}$ \textsc{B}{\textbar} $\langle$ some contextually defined number}
\ex    {Both students are brilliant}.\\ \jambox{S ${\subseteq}$ B  \& \textsc{{\textbar}S}{\textbar} = 2}
% \lspbottomrule
\z \z
...
\ea
\textit{Every Roman is patriotic}.\\
\z
...
\ea
{Some wealthy Romans are patriotic}.
\z
\ea
\z
\ea
{Both Romans are patriotic}.
\z
\ea
{Caesar loves all Romans who obey him.}
\z
\ea
{Most loyal Romans love Caesar.}
\z
...
\z
\ea
Many theologians do not understand this doctrine.
\z
\ea
This doctrine is not understood by many theologians.
\z
\ea
Two-thirds of the members did not vote for the amendment.
\z
\ea
...
\z
\ea
He tries to read Plato’s \textit{Republic} every year.\footnote{Marilyn Quayle, on the reading habits of her husband; \textit{Wall Street Journal}, January 20, 1993.}
\z
...
% 
\ea
Solomon answered every riddle.
\z

\modelanswer{Model answer}{${\forall}$ x[RIDDLE(x) → ANSWER(s,x)]}
 
\ea
All ambitious politicians visit Paris.
\z
\ea
Someone betrayed Caesar.
\z
\ea
All critical systems are not working.
\z
\ea
No German general supported Stalin.
\z
\ea
Not every German general supported Hitler.
\z
\ea
Some people believe every wild rumor.
\z
\ea
Socrates inspires all sincere scholars who read Plato.
\z
...
\ea
Arthur eats everything that Susan cooks.
\z
\modelanswer{Model answer}{[Every x: THING(x) \& COOK(s,x)] EAT(a,x)}

\ea
Donald mistrusts most reports from Brussels.\\
{}[hint: treat \textit{from} as a two-place predicate]
\z
\ea 
Few who know him like Arthur.
\z
\ea 
William sold Betsy every arrowhead that he found.
\z
\ea 
Twenty-one movies were directed and produced by Alfred Hitchcock.
\z
\ea 
Most travelers entering or leaving Australia visit Sydney.
\z
\ea 
No one\textsubscript{i} remembers every promise he\textsubscript{i} makes.
\z
\ea 
Some officials who boycotted both meetings were sacked by Reagan.
\z
\ea 
Jane Austen and E. M. Forster wrote six novels each.
\z
\ea 
Rachel met and interviewed several famous musicians.
\z
\ea 
Most children will not play if they are sad.
\z
...
\todo{underlining lost}

\ea
More than twenty  senators are guilty.   
\z
\modelanswer{Model answer}{{{\textbar}} $\llbracket${SENATOR}$\rrbracket$  { ${\cap}$} $\llbracket${GUILTY}$\rrbracket$ {{\textbar} > 20}}

\ea
Between six and twelve generals are loyal.
\z
\ea 
Both sisters are champions.
\z
\ea
The twelve apostles were Jewish.
\z
\ea
Just two of the seven guides are bilingual.
\z
\ea 
Neither candidate is honest.
\z
\ea 
Fewer than five crewmen are sober.
\z
...
\z
\ea 
Exactly as many Americans are lawyers as are prisoners.\footnote{Actually the figures are only approximately equal, but there are clearly too many of both.} 
\z
\ea
Fewer wrestlers than boxers are famous. 
\z
}
