...
\ea \label{ex:16.1}
\ea \label{ex:16.} Arthur \textit{must/has to} be home by now.\\
...
\ea \label{ex:16.2}
\ea \label{ex:16.} John didn’t show up for work. He \textit{must} be sick. \hfill [spoken by co-worker; Epistemic]\\
\ex John didn’t show up for work. He \textit{must} be fired.               \hfill [spoken by boss; Deontic]
                       \z
\z

\ea \label{ex:16.3}
\ea \label{ex:16.} The older students \textit{may} leave school early (unless the teachers watch them carefully).\\
...
\ea \label{ex:16.5}
\ea \label{ex:16.}  Il est 18 heures. Anne n’est pas au bureau. Elle \textit{peut/doit} être chez elle.\\
...
\ea \label{ex:16.6}
\ea \label{ex:16.} You \textit{should}/\textit{ought} \textit{to} call your mother, but of course you don’t \textit{have to}.\\
\ex \#You \textit{have to} call your mother, but of course you \textit{shouldn’t}.
\z \z

\ea \label{ex:16.7}
\ea \label{ex:16.} I \textit{should} go to confession, but I’m not going to.\\
\ex \#I \textit{must} go to confession, but I’m not going to.
                       \z
\z

\ea \label{ex:16.8}
\ea \label{ex:16.} Arthur \textit{should} be home by now, but he doesn’t \textit{have to} be.\\
...
\ea \label{ex:16.9}
\ea \label{ex:16.} \#Arthur \textit{must/has to} be home by now, but I consider it unlikely.\\
  (bad on epistemic reading)\\
...
In \chapref{sec:14} we introduced two symbols from modal logic: ${\lozenge}$ = ‘it is possible that’; and ${\square}$ = ‘it is necessarily the case that’. The use of these symbols is illustrated in the logical forms for two simple modal statements in \REF{ex:16.}.


\ea \label{ex:16.11}
\ea \label{ex:16.} \textit{Arthur must be at home}. \hfill logical form: ${\square}$ AT\_HOME(a)\\
...
\ea \label{ex:16.12}
\ea \label{ex:16.} \textit{Arthur must be at home}. \hfill meaning: ${\forall}$w[AT\_HOME(a) in w]\\
...
\ea \label{ex:16.15}
\ea \label{ex:16.}   (${\lozenge}$ p)  $\leftrightarrow $  ¬(${\square}$ ¬p)\\
\ex   (${\square}$ p)  $\leftrightarrow $  ¬(${\lozenge}$ ¬p)
                       \z
\z

\ea \label{ex:16.16}
\ea \label{ex:16.}   (${\exists}$x[P(x)])  $\leftrightarrow $  ¬(${\forall}$x[¬P(x)])\\
...
\ea \label{ex:16.} ${\forall}$x[STUDENT(x) → ${\lozenge}$ FAIL(x)]\\
...
\ea \label{ex:16.} ${\exists}$x[SANCTION(x) $\wedge$ ${\square}$ BE-IMPOSED(x)]\\
...
\ea \label{ex:16.19}
\ea \label{ex:16.} \textit{Arthur must go to prison}.  [Deontic]\\
\ex{} [all w: w is consistent with what the law requires] GO\_TO\_PRISON(a) in w
                       \z
\z

\ea \label{ex:16.20}
\ea \label{ex:16.} \textit{If Arthur has robbed a bank, he} \textit{must go to prison}.\\
...
\ea \label{ex:16.21}
\ea \label{ex:16.} \textit{The state may confiscate Arthur’s assets}.  [Deontic]\\
...
\ea \label{ex:16.23}
\ea \label{ex:16.} \textit{Arthur must be at home}.  [Epistemic]  (=\ref{ex:16.13}a)\\
...
\ea \label{ex:16.24}
\ea \label{ex:16.} \textsc{Epistemic:}\\
...
\ea \label{ex:16.25}
\ea \label{ex:16.}  \gll wá7=\textbf{k’a}  s-t’al  l=ti=tsítcw-s=a  s=Philomena\\
...
\ea \label{ex:16.27}
\ea \label{ex:16.} \gll Inna-jaa-\textit{ssa}-atit.\\
...
\ea \label{ex:16.29}
\ea \label{ex:16.} Henry must be in Brussels this week.  \hfill [epistemic or deontic]\\
...
\ea \label{ex:16.30}
\ea \label{ex:16.} Mary must attend Prof. Lewis’s lecture every week. \hfill [epistemic or deontic]\\
...
\ea \label{ex:16.31}
\ea \label{ex:16.} Mary may attend Prof. Lewis’s lecture every week. \hfill [epistemic or deontic]\\
...
\ea \label{ex:16.41}
\ea \label{ex:16.}  \gll Er  \textit{muss  nicht}  zu  hause  bleiben.\\
...
\ea \label{ex:16.42}
\ea \label{ex:16.} \gll Dia  \textit{mesti  tidak}  belajar\\
...
A number of authors have cited these examples in support of the claim that epistemic modality always takes scope over clausal negation.\footnote{See for example \citet{deHaan1997}; \citet{Drubig2001}.} However, corpus examples like those in \REF{ex:16.43} show that the epistemic use of \textit{mesti} is in fact possible within the scope of clausal negation.


\ea \label{ex:16.43}
\ea \label{ex:16.} \gll Inflasi  \textit{tidak  mesti}  ber-punca  dari  pemerintah…\\
...
\item \label{ex:16.} \textit{You must leave tomorrow}.
...
\ea \label{ex:16.}
 \ea \label{ex:16.} \textit{Arnold must trust you}.  (assume “h” = hearer)\\
\textsf{Model answer:\\
...
\ea \label{ex:16.}
\ea \label{ex:16.} \textit{No terrorist must enter the White House}.  [deontic]\\
Model answer:\\
