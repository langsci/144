...
\label{extab:13.21}
\fittable{
\begin{tabular}{l >{\itshape}lllc}
\lsptoprule
 & English sentence &  logical form &  interpretation &  truth value\\
\midrule
a. &  Thomas More is a man. &  MAN(t) & Thomas More ${\in}$ \textsc{$\llbracket$}MAN$\rrbracket$  &   T \\

b. &  Anne Boleyn is a man or a woman. &  MAN(a) $\vee$ WOMAN(a)  &  Anne Boleyn ${\in}$ (\textsc{$\llbracket$}MAN$\rrbracket$  ${\cup}$ \textsc{$\llbracket$}WOMAN$\rrbracket$ ) & T\\

c. &   {Henry VIII is a man who snores}. & MAN(h) $\wedge$ \textsc{SNORE}(h) & Henry VIII ${\in}$ (\textsc{$\llbracket$}MAN$\rrbracket$  ${\cap}$ \textsc{$\llbracket$SNORE}$\rrbracket$ ) & T\\

d. & {All men snore}.& ${\forall}$x[MAN(x) → \textsc{SNORE}(x)] & \textsc{$\llbracket$}MAN$\rrbracket$  ${\subseteq}$ \textsc{$\llbracket$SNORE}$\rrbracket$  & F\\

e. &   {No women snore.} &  ¬${\exists}$x[WOMAN(x) $\wedge$ \textsc{SNORE}(x)] & \textsc{$\llbracket$WOMAN}$\rrbracket$  ${\cap}$ \textsc{$\llbracket$SNORE}$\rrbracket$  = ∅ & T\\
\lspbottomrule
\end{tabular}
}  
\end{table}
...
\tablevspace
d. &  the set containing all members of the Beatles& people with 2-syllable first names\\
\tablevspace
...
\ex denotation assignments\textsc{:\\
...
\textsc{$\llbracket$}a$\rrbracket$  = Able    \\
\textsc{$\llbracket$}b$\rrbracket$  = Baker   \\
\textsc{$\llbracket$}c$\rrbracket$  = Charlie \\
\textsc{$\llbracket$}d$\rrbracket$  = Doug\\
  \textsc{$\llbracket$}e$\rrbracket$  = Echo\\
  \textsc{$\llbracket$}f$\rrbracket$  = Fred\\
 \textsc{$\llbracket$}g$\rrbracket$  = Geronimo\\
    \z \z
...
\tablevspace
b. & \textit{Doug is a blue fish}.    & BLUE(d) $\wedge$ \textsc{FISH}(d) & Doug ${\in}$ (\textsc{$\llbracket$}BLUE$\rrbracket$  ${\cap}$ \textsc{$\llbracket$FISH}$\rrbracket$ ) & T  \\
\tablevspace
...
\ea
\ea For each of the following logical formulae, provide an English translation and an interpretation stated in terms of set notation.\\
\ex Create a model under which sentences (\ref{ex:13:hw:ref}b--c) will be false, and the rest (including (\ref{ex:13:hw:ref}a)) will be true.\footnote{Patterned loosely after \citet[350]{Saeed2009}.}
\z
\z
...
\modelanswer{Model answer}{English translation: ‘Mr. Darcy loves/loved Elizabeth.’\\
truth conditions: $\langle$\textsf{Darcy, Elizabeth}$\rangle$  ${\in}$  $\llbracket$\textsf{LOVE}$\rrbracket$ 
}
...
\z
\ea
${\forall}$x [(MAN(x) $\wedge$ WEALTHY(x)) → ADMIRE(b,x)]
\z
\ea
${\exists}$x [MAN(x) $\wedge$ WEALTHY(x) $\wedge$ ADMIRE(b,x)]
\z
\ea
¬${\exists}$x [WOMAN(x) $\wedge$ LOVE(x,c)]
\z
\ea
DECEIVE(w,l) $\wedge$ RESCUE(d,l)
\z
\ea
${\forall}$x [WOMAN(x) → CHARM(w,x)] $\wedge$ ${\forall}$y [MAN(y) → ANGER(w,y)]
\z
}
